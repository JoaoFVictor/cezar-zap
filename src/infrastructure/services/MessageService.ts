...
export class MessageService {
    constructor(@inject("ClientWhatsApp") private client: Client, private messageRepository: MessageRepository, private cacheService: CacheService) {}

    async createMessage(message: Message): Promise<Message> {
        return await this.messageRepository.createMessage(message);
    }

    async sendMessage(phoneNumber: string, content: string, sendBaseCommands: boolean = true, additionalCommands: string|null = null): Promise<void> {
        if (sendBaseCommands) {
