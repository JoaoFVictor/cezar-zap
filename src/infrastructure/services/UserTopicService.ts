...
import UserTopicState from '../../application/entities/enums/UserTopicState';
import { injectable } from 'tsyringe';

@injectable()
export class UserTopicService {
    constructor(private userTopicRepository: UserTopicRepository, private cacheService: CacheService) {}
    ...
    public async findTopLevelTopicByOption(user: User, option: string): Promise<UserTopic | null> {
        return await this.cacheService.remember(`find_top_level_topic_by_option_${option}_and_user_id_${user.id}`, CacheTimes.ONE_DAY, async () => {
    ...
    public async findChildTopicByOption(user: User, parentTopic: UserTopic, option: string): Promise<UserTopic | null> {
        return await this.cacheService.remember(`find_child_topic_by_option_${option}_and_parent_topic_${parentTopic.id}_user_id_${user.id}`, CacheTimes.ONE_DAY, async () => {
            return await this.userTopicRepository.findChildTopicByOption(user, parentTopic, option);
        });
    }
    
    public async getUserTopicStage(user: User): Promise<{ topLevelUserTopics: UserTopic[], currentUserTopic: UserTopic | null, userTopicStack: UserTopic[] } | null> {
        return await this.cacheService.get<{ topLevelUserTopics: UserTopic[], currentUserTopic: UserTopic | null, userTopicStack: UserTopic[] }>(`get_user_topic_stage_by_user_id_${user.id}`);
    }

    public async setUserTopicStage(user: User, state: { topLevelUserTopics: UserTopic[], currentUserTopic: UserTopic | null, userTopicStack: UserTopic[] }): Promise<void> {
    ...
    public async setUserTopicState(user: User, userTopicState: UserTopicState): Promise<void> {
        await this.cacheService.put(`user_${user.id}_topic_state`, userTopicState, CacheTimes.THIRTY_MINUTES);
    }

    public async setTemporaryUserTopic(user: User, userTopic: UserTopic): Promise<void> {
    ...
    public async finalizeTopicCreation(user: User, description: string): Promise<UserTopic> {
